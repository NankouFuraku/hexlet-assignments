...
class AppTest {
    ...
    @Test
    void testReversed() {
        var text = "NankouFuraku";
        var actual = new ReversedSequence(text);

        assertThat(actual.toString()).isEqualTo("ukaruFuoknaN");
        assertThat(actual.charAt(2)).isEqualTo('a');
        assertThat(actual.length()).isEqualTo(12);
        assertThat(actual.subSequence(1, 4)).isEqualTo("kar");
    }
    // END
}
